...
export function ContentMainView(props: ContentMainViewProps) {
  ...
  const inlineEditing = useInlineMarkdownEditing({
    enabled: !activeHistory && documentSession?.mode === 'inline-edit',
    bodyRef,
      ...
      <div className={`content__scroll${isFullHtmlPreview ? ' content__scroll--html-preview' : ''}`} id="contentScroll" ref={scrollRef}>
        ...
        {!state.isLoading && workspaceUnavailablePath && (
          <div className="state-screen state-screen--workspace-unavailable">
            <div className="state-screen__icon state-screen__icon--warning"><AlertTriangleIcon size={34} /></div>
            <div className="state-screen__title">{t.workspaceUnavailable.title}</div><div className="state-screen__sub">{t.workspaceUnavailable.description}</div>
            <div className="state-screen__path">{workspaceUnavailablePath}</div>{isDesktopTabView && <div className="state-screen__hint">{t.workspaceUnavailable.tabHint}</div>}
        ...
        {!state.isLoading && !state.isWorkspaceScanning && !state.notFoundHref && !workspaceUnavailablePath && state.fileList.length === 0 && !state.contentHtml && (
        ...
        {!state.isLoading && !state.notFoundHref && !workspaceUnavailablePath && state.currentFile && hasRenderableDocumentContent && (
          <div className={`mdn-body${isFullHtmlPreview ? ' mdn-body--html-preview' : ''}`} id="mdBody" ref={bodyRef} aria-live="polite" data-mdn-source-document-path={state.relativePath || undefined}>
            ...
            {activeHistory?.mode === 'git-revision' && activeHistory.revision ? (
              <GitRevisionView snapshot={activeHistory.revision} onReturnToCurrent={() => clearHistoryView('single')} />
            ) : activeHistory?.mode === 'diff' && activeHistory.comparison ? (
              <DocumentDiffView {...activeHistory.comparison} onReturnToCurrent={() => clearHistoryView('single')} />
            ) : isHtmlDocument && sourceDocumentText !== null ? (
              <HtmlDocumentView filePath={state.currentFile} htmlSource={sourceDocumentText} markdownHtml={htmlMarkdownRender.html} previewEnabled={htmlDocumentPreviewEnabled} title={state.relativePath || state.currentFile} conversionError={htmlMarkdownRender.error} onPolicyReport={onHtmlPolicyReport} />
            ) : isPlainMarkdownMode && documentSession ? (
              <PlainMarkdownEditor value={documentSession.source} disabled={documentSession.saveState === 'saving'} ariaLabel={editorT.plainSourceLabel} onChange={(source) => onWorkingDocumentSourceChange?.(state.currentFile!, source)} onSave={() => { void onSaveDocument?.(state.currentFile!); }} />
            ) : (
              <>
                {previewInfo && <div className={`document-preview-notice document-preview-notice--${previewInfo.kind}`} role="note"><AlertTriangleIcon size={16} /><div className="document-preview-notice__body"><div className="document-preview-notice__title">{previewTitle}</div><div className="document-preview-notice__text">{previewWarning}</div>{previewMeta && <div className="document-preview-notice__meta">{previewMeta}</div>}</div></div>}
