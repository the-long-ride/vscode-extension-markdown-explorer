...
function SplitPaneView({ paneId, projection, language, renderVersion, onModeChange, onSourceChange, onSave, onScrollChange }: PaneViewProps): ReactNode {
  ...
  const historyView = projection && historyViews[paneId]?.filePath === projection.filePath ? historyViews[paneId] : undefined;
  ...
  const modes: readonly DocumentViewMode[] = historyMode && projection.mode === historyMode
    ...
    : historyMode ? [...EDITABLE_MODES, historyMode] : EDITABLE_MODES;
    ...
    <div className="split-document-pane__content">
      <header className="split-document-pane__header">
        ...
        <div className="split-document-pane__modes" role="group" aria-label={`${paneLabel}: ${editorT.modeGroup}`}>
          {modes.map((mode) => (
            <button key={mode} type="button" className={`split-document-pane__mode${projection.mode === mode ? ' is-active' : ''}`} aria-label={`${paneLabel} ${modeLabels[mode]}`} aria-pressed={projection.mode === mode} onClick={() => {
      ...
      <div ref={scrollRef} className="split-document-pane__scroll" data-testid={`split-pane-scroll-${paneId}`} onScroll={(event) => onScrollChange(paneId, event.currentTarget.scrollTop)}>
        {projection.mode === 'git-revision' && historyView?.revision ? (
          <GitRevisionView snapshot={historyView.revision} onReturnToCurrent={() => clearHistoryView(paneId)} />
        ) : projection.mode === 'diff' && historyView?.comparison ? (
          <DocumentDiffView {...historyView.comparison} onReturnToCurrent={() => clearHistoryView(paneId)} />
        ) : (
          <DocumentSurface filePath={projection.filePath} relativePath={projection.relativePath} mode={projection.mode === 'git-revision' || projection.mode === 'diff' ? 'rendered' : projection.mode} contentHtml={projection.contentHtml} source={projection.source} stale={false} language={language} renderVersion={renderVersion} disabled={projection.session?.saveState === 'saving'} onSourceChange={(source) => onSourceChange(projection.filePath, source)} onSave={() => onSave(projection.filePath)} />
        )}
      </div>
    </div>
  );
...
export function SplitContentView({ state, onActivatePane, onRatioChange, onCloseSplit, onModeChange, onSourceChange, onSave, onScrollChange }: SplitContentViewProps) {
  ...
  return (
    <main className="content content--split" id="mainContent">
      <SplitDocumentView
        ratio={state.splitView.ratio} activePane={state.splitView.activePane}
        primaryLabel={splitT.primaryDocument} secondaryLabel={splitT.secondaryDocument}
        closeSecondaryLabel={splitT.closeSecondaryPane} resizeLabel={splitT.resizeDocumentPanes}
        primary={<SplitPaneView paneId="primary" projection={primary} language={language} renderVersion={state.renderVersion} onModeChange={onModeChange} onSourceChange={onSourceChange} onSave={onSave} onScrollChange={onScrollChange} />}
        secondary={<SplitPaneView paneId="secondary" projection={secondary} language={language} renderVersion={state.renderVersion} onModeChange={onModeChange} onSourceChange={onSourceChange} onSave={onSave} onScrollChange={onScrollChange} />}
        onActivatePane={onActivatePane} onRatioChange={onRatioChange} onCloseSecondary={onCloseSplit}
      />
    </main>
  );
}
